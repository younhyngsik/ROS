...
해상도나 좌표는 x, y(width, height) 인데 초기화나 shape는 y, x(행, 열 순서)
...
rgb 컬러공간 => 삼원색을 정육면체 모델로 표현
hsv 컬러공간 => 이미지 처리에서 가장 많이 사용 -> 색, 채도, 명도로 색을 표현
		-> h(ue): 360가지의 색을 구분할 수 있지만 실제로는 180까지 측정 후 2배 해서 계산 -> 255까지가 8비트라

-----------------------------------------------------------------------
0805
draw - ellipse, marker(*, # 등 기호), polygon, mask된 부분 -> polygon 으로 구현
event => mouse -> callback, keyboard -> poll, waitkey(특수키 x), waitkeyEx(특수키 포함)

어플 => opencv보단 pyQt나 tkinter(py), pywebview(html) -> 1순위는 pywebview

mask => 이미지 결합 -> bitwise(비트 연산), copyto, add
픽셀 수정 => roi -> sliding(numpy) [a:b][c:d] (세로줄이 a~b) => 참조가 기본 / 깊은 복사 -> copy()

color_space => BGR -> RGB, HSV, ... 로 변환,
trackbar -> 윈도우 설정
=> 종합해서 gazebo에서 빨간 공 따라가는 미니 프로젝트
...
영상의 크기 변환 => 이미지의 픽셀 사이의 간격을 넓히고 빈 공간(홀)에 인접 픽셀의 평균값으로 보간
회전 => 회전의 기준점으로 영상을 이동시킨 후,  회전 수행 -> 다시 원점 좌표로 이동
원근 변환 = > cv::getPerspectiveTransform: 4개의 좌표로부터 원근변환 행렬을 계산


aruco 상자 위치에 좌표축 그리기
상자 상단의 tf 발행해서 tvec, rvec 발행(dynamic tf)
-> 카메라 subscription, aruco 추가, tf 발행, imshow를 통해 좌표가 잘 그려지는지 확인


--------------------------------------------------------------
0806
opencv 변환 => affine transform(비율, 이동, 회전 ...) -> 보간 기법으로 보정
perspective transform => 시점 변환
3차원에서의 물체 인식 aruco
...
필터: 컨볼루션 연산을 통해 주변 픽셀과의 관계를 산출
box filter, blur, filter2D: 블러 필터
gausian filter:
median filter: 특정 픽셀의 노이즈를 평균값으로 제거(솔트 & 페퍼같은 노이즈)
bilateral filter: 경계를 살리면서 노이즈 제거
미분 필터
Sobel filter: 윤곽선 따는 필터 -> 노이즈 제거하려면 필터 적용 후 threshold 한번 거치기..?
mophology: 영상을 형태학적으로 접근해 객체 외각을 깎아내거나 객체 외곽(잡음) 제거
		->열기: 침식 후 팽창 -> 작은 돌기, 객체 사라지고 얇은 선 끊어짐
		->닫기: 팽창 후 침식 -> 작은 홀 사라지고 두꺼운 선 얇아짐

Canny edge: 가우시안 블러링(노이즈 제거) -> 소벨 마스크(화소 기울기의 강도, 방향 검출) -> 비최대치 억제 -> 이력 임계값으로 에지 결정
	-> 가장 보편적으로 쓰임

이미지에서 선의 기울기는 직각 좌표계가 아니라 극좌표계로 구함 -> 직각좌표계는 직각인 선의 기울기가 무한에 가깝게 커짐
Hough 변환: 각 점마다 지나는 직선의 빈도를 검출하고 이를 통해서 겹치는 정도에 따라 정확도를 조절 가능

코너 검출: 코너 갯수와 상대 위치를 통하여 객체 인식
직선 검출: 직선 갯수와 ~
패턴 체크: 물체 표면의 패턴 갯수와 ~ => cascade 방식 -> 멀거나 물체가 돌아가면 인식 힘듦
디스크립터: 벡터 맵

Harris 코너 검출: 코너 검출에서 window를 가우시안 마스크로 적용
->소벨 마스크로 미분 행렬 계산 -> 미분 행렬 곱 계산 -> 곱 행렬에 가우시안 마스크 적용 -> 코너 응답 함수 계산 -> 비파괴 억제 수행
정확하지만 느림

FAST 코너 검출: 주변 16개 픽셀 값들을 조사해서 가운데 픽셀보다 충분히 어둡거나 밝으면 코너로 검출
GFTT: 해리스 코너 검출의 업그레이드 버전, 조금 더 빠름
ORB: FAST는 회전할 시 일관성이 없어서 이를 개선, 성능 조금 떨어지나 빠름
...
---------------------------------------------------------------
0807
객체 검출: convolution -> 미분 필터 -> edge 검출 -> 라인 검출, 코너 검출 -> 객체 인식 
코너 검출: Harris, ORB, precorner, eigen, gftt...
morphology => mask 영상 편집
...
인공지능 -> argument에서 threshold나 ksize 같은 변수를 알아서 상황에 맞게 조절
머신러닝: 	학습: 학습데이터 -> 가공 -> 평가 및 피드백 -> 모델
		실제 추론: 새로운 데이터 -> 모델 -> 예측
		(지도학습(회귀: 결과가 숫자, 분류): 정답 o, 비지도 학습(군집화): 정답 x, 강화학습
		검증: 데이터가 label을 통과해서 나온 결과와 실제 추론(예측) 결과를 비교해서 정답률 계산
CNN: convolution Neural Network: 컨볼루션층, 풀링, 신경망으로 구성
컨볼루션층: 이미지 필터링을 위한 연산 => 필터를 이용하여 입력 이미지에 필터의 특징이 있는지 검사
풀링: 세로 및 가로 방향으로 이미지 크기를 줄이는 연산, 하지만 커널 갯수는 유지
Max Pooling: 예) 4 * 4 이미지를 2*2 필터를 각 모서리에 적용하여 4칸중 가장 큰 값으로 일반화하여 2*2로 축소

uv 가상환경 실행
source /home/youn/ROS/perception_envs/depth_yolo/.venv/bin/activate
비실행: deactivate

python3 => install에 lib의 1번 라인 변경
/home/youn/ROS/perception_envs/depth_yolo/.venv/bin/python3.12
